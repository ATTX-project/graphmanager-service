...
class GraphUpdate(object):
    ...
    def on_post(self, req, resp, parsed):
        ...
        fuseki = GraphStore()
        resp.data = json.dumps(fuseki.graph_add(parsed['targetGraph'], parsed['triples'], parsed["contentType"]))
        resp.content_type = 'application/json'
        resp.status = falcon.HTTP_200
...
class GraphSPARQL(object):
    ...
    def on_post(self, req, resp, parsed):
        ...
        fuseki = GraphStore()
        data = fuseki.graph_sparql(parsed['targetGraph'], parsed['query'], parsed["contentType"])
        resp.data = str(data)
