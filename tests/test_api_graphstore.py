...
class GraphTestCase(GraphStoreTest):
    ...
    def test_api_update_no_data(self):
        ...
        result = self.simulate_post('/{0}/graph/update'.format(self.version), body='', headers=hdrs)
    ...
    def test_api_add_bad(self):
        ...
        hdrs = [('Accept', 'application/json'),
                ('Content-Type', 'application/json'), ]
        result = self.simulate_post('/{0}/graph/update'.format(self.version), body=graph_data, headers=hdrs)
        assert(result.status == falcon.HTTP_400)
